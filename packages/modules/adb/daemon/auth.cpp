 ...
 */
 ...
 // Ankit: uncomment to use original
 ...
 void adbd_auth_confirm_key(atransport* t) {
     // Check if the key is pre-authorized
     bool key_authorized = false;
     IteratePublicKeys([&](std::string_view public_key) {
         std::vector<std::string> split = android::base::Split(std::string(public_key), " \t");
         if (!split.empty() && split[0] == t->auth_key) {
             key_authorized = true;
             return false; // Stop iteration if key matches
         }
         return true; // Continue
     });
 
     if (key_authorized) {
         // Key is authorized: proceed without dialog
         LOG(INFO) << "Key authorized automatically";
         adbd_auth_verified(t);
     } else {
         // Key is NOT authorized: disconnect immediately
         LOG(WARNING) << "Blocking unauthorized key: " << t->auth_key;
         t->Kick(); // Terminate the connection
     }
 }
     ...
     std::thread([t]() {
         ...
         if (t->connection()->DoTlsHandshake(rsa_pkey, &auth_key)) {
             ...
             if (t->IsTcpDevice()) {
                 ...
                 IteratePublicKeys([&](std::string_view public_key) {
                     std::vector<std::string> split = android::base::Split(std::string(public_key), " \t");
                     if (!split.empty() && split[0] == auth_key) {
                         key_authorized = true;
                         return false;
                     }
                     return true;
                 ...
                 if (key_authorized) {
                     ...
                     LOG(WARNING) << "Blocking unauthorized TLS key: " << auth_key;
                     t->Kick();
                 }
             }
         } else {
